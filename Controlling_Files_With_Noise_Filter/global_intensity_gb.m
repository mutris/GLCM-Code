...
a = unique(image_store_gb_before);
s = size(image_store_gb_before);
f = zeros(length(a),1);
sum_m = zeros(length(a),1);

for i = 1: length(a)
    c = find(image_store_gb_before == a(i));
    f(i) = length(c);
end
...
clear x i j
num_img = 40;
image_store_gb_after = zeros(400,400,num_img);
current_image = zeros(400,400);
new_image = zeros(400,400);

for x = 1:40

    current_image = image_store_gb_before(:,:,x);
    for i = 1:400
        for j = 1:400
    ...
    image_store_gb_after(:,:,x) = new_image;
end
...
clc
test_auto_minerals_gb = zeros(400,400,40);
test_auto_pores_gb = zeros(400,400,40);
num_img = 40;
...
for x = 1:40 
    ...
    test_auto_minerals_gb(:,:,x) = im_conv_high;
    ...
    test_auto_pores_gb(:,:,x) = im_conv_low;    
end

%%

save gb_after_contrast.mat image_store_gb_after test_auto_minerals_gb test_auto_pores_gb 
